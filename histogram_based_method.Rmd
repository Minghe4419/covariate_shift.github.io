...
# Choose parameters
r = 0.5 # we may use monte-carlo simulation
...
# Generates random mean and covariance for a single Gaussian component
generate_gaussian_params <- function(d = 2, seed = NULL) {
  #if (!is.null(seed)) set.seed(seed)
  
  # Random mean between -3 and 3 in each dimension
  mu <- runif(d, min = -3, max = 3)
  
  # Random positive definite matrix for covariance
  # 1. Generate random d x d matrix M
  # 2. Make it symmetric positive-definite via Sigma = M^T * M + epsilon * I
  M <- matrix(rnorm(d^2), nrow = d, ncol = d)
  Sigma <- crossprod(M) + diag(d) * 0.1  # ensures positivity + some minimum variance
  
  list(mu = mu, Sigma = Sigma)
}

# Generates mu1, mu2, sigma1, sigma2 for a mixture of two Gaussians
generate_mixture_params <- function(d = 2, seed = NULL) {
  #if (!is.null(seed)) set.seed(seed)
  
  comp1 <- generate_gaussian_params(d)
  comp2 <- generate_gaussian_params(d)
...
# Generate random mixture parameters for source
source_params <- generate_mixture_params(d = d, seed = 123)

# Generate random mixture parameters for target
target_params <- generate_mixture_params(d = d, seed = 456)
...
select_optimal_r <- function(source_data, target_data, d, n0, n1,
  ...
  beta_values <- runif(beta_samples, min = 1e-3, max = 1)  # Avoid exactly 0
