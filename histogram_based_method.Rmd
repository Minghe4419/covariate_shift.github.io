...
simulate_data <- function(n1 = 1000,   # number of target samples
  ...
  mu_source1 <- rep(-4, d)
  mu_source2 <- rep(4, d)  # shift second component to separate it somewhat
  
  # Covariance matrices for the two source components (identity for simplicity)
  Sigma_source1 <- diag(d)*4
  Sigma_source2 <- diag(d)*4
  ...
  p_target <- 0.5  # mixture proportion for target data
  mu_target1 <- rep(-4,  d)
  mu_target2 <- rep(4, d)
  Sigma_target1 <- diag(d)
  Sigma_target2 <- diag(d)
...
*Note for myself*: what is the test size the auhtor mentioned in Firgure 2 of the paper? what does X = t * x mean? How come distribution of t 1D? 
...
# Choose parameters
r = 0.02 # we may use monte-carlo simulation
#r = 0.5731
...
test_n = 2000
test_component <- sample(1:2, size = test_n, replace = TRUE)
test_query <- ifelse(test_component == 1,
                   rnorm(test_n, mean = -4, sd = 4),
                   rnorm(test_n, mean = 4, sd = 4))
test_matrix <- matrix(test_query, nrow = test_n, ncol = dimension)


estimations = matrix(NA, nrow = test_n, ncol = 1)
weighted_ratio = matrix(NA, nrow = test_n, ncol = 1)
for (i in seq_len(test_n)) {
  weighted_ratio[i, ] <- compute_histogram_ratio(test_matrix[i, ], target_data, source_data, r)$ratio
  estimations[i, ] <- compute_histogram_ratio(test_matrix[i, ], target_data, source_data, r)$estimation
}
...
Here we simulate the data-driven method used in [Kpotufe(2017)](https://proceedings.mlr.press/v54/kpotufe17a/kpotufe17a.pdf).

The optimal rate $r$ is `r optimal_selection$optimal_r`.